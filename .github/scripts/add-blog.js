...
try {
  ...
  if (!Array.isArray(data.tags)) {
    console.error(
      'Tags must be an array in frontmatter, e.g.:\ntags: ["tag1", "tag2"]',
    );
    process.exit(1);
  }
  ...
  if (!mdUrl) {
    ...
    if (!normalizedPath.includes("/blogs/")) {
      console.error(
        "Error: File must be in a subdirectory of blogs/ or specify mdUrl in frontmatter",
      );
      console.error(
        "Valid directories: blogs/tech/, blogs/science/, blogs/maths/, blogs/music/, blogs/history/",
      );
      process.exit(1);
    }
    ...
    !mdUrl.startsWith("https://raw.githubusercontent.com/Sivothajan/")
  ) {
    console.error(
      "mdUrl must be a GitHub raw URL starting with https://raw.githubusercontent.com/Sivothajan/",
    );
    process.exit(1);
  }
  ...
  if (existsSync("index.json")) {
    posts = JSON.parse(readFileSync("index.json", "utf8"));
  } else {
  ...
  const post = {
    id:
      existingIndex >= 0 ? posts[existingIndex].id : getNextAvailableId(posts),
    title: data.title,
    date: data.date,
  ...
  writeFileSync("index.json", JSON.stringify(posts, null, 4));
} catch (error) {
  console.error("Error:", error.message);
  if (error.code === "ENOENT") {
    console.error(
      "Make sure you are running the script from the repository root directory",
    );
  }
  process.exit(1);
}
